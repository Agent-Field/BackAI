...
func TestDemoProviderChatReturnsSupportDeskReplyAndUsage(t *testing.T) {
	...
	if !strings.Contains(got, "billing review") {
...
func TestDemoProviderDoesNotEchoAgentFieldPlan(t *testing.T) {
	provider := NewDemoProvider()
	resp, err := provider.Chat(context.Background(), ChatRequest{
		Model: "demo/supportdesk",
		Messages: []ChatMessage{{
...
A customer says their invoice is wrong and wants a refund.

AgentField support plan:
{"agent":"supportdesk","reasoners":["classify_issue","refund_guardrail"]}`,
		}},
	})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	...
	got, _ := resp.Choices[0].Message.Content.(string)
	if strings.Contains(got, "AgentField support plan") || strings.Contains(got, "classify_issue") {
		t.Fatalf("reply leaked hidden plan context:\n%s", got)
	}
	if !strings.Contains(got, "invoice is wrong") {
		t.Fatalf("reply missing ticket context:\n%s", got)
	}
}
