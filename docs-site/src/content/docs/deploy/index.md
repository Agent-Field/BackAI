---
title: Deploy AF Stack
description: Five officially-supported deploy targets. Pick the one matching your ops posture.
sidebar:
  order: 1
  label: Overview
---
...
| `OPENROUTER_API_KEY`         | LLM provider (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`) |
...
See [Backup & Restore](/reference/backup-restore/) for:
...
- Architecture: `PRD.md`, `TECH-SPEC.md` (repo root).
- Sandbox adapters trade-offs: [Sandbox adapters](/reference/adapters/sandbox/)
- Backup + restore: [Backup & Restore](/reference/backup-restore/)
- Multi-replica scaling: [Multi-replica](/deploy/multi-replica/)
- Production internals: [Production internals](/deploy/internals/)
