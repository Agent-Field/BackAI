...
| **Billing**           | The billing integration _your customers_ see (Stripe / Lago): plans, metered metrics, webhook handlers                        |
...
customer sign-up, help center, support chat, request history, customer
billing, and account settings. Platform evidence stays in the operator
dashboard: cost/request evidence, run metadata, registered agents, and
service UI links. Heavier examples such as Notable and Shipwright remain
examples, not the default product path.
