...
rotate-kms`, and `AF_STACK_KMS_KEY_NEW` is read by nothing (see
[the Helm chart README](../deploy/helm/af-stack/README.md)). There is no
dual-key window, so **swapping the key before you have exported the
values makes every row in `suite_secrets` permanently unrecoverable.**
