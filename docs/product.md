...
| **Sandbox runs**             | Docker adapter for dev (real containers spun up via host's docker.sock, stdout/stderr captured to MinIO). gVisor / Firecracker / e2b adapters for production, plus a `remote` adapter for any out-of-process sandbox sidecar.                                                                                                           |
...
| **Harnesses**                | Probe-only — detects whether claude-code/codex/gemini/opencode is available in the agent container and what auth it needs. Surfaced by the operator dashboard's harness cards and `GET /api/v1/harnesses`, `GET /api/v1/harnesses/{provider}`, `POST /api/v1/harnesses/{provider}/probe`. There is no `af-stack harness` command on the operator CLI.                                                                                                                                              |
