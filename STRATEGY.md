...
For the layered stack diagram and OSS placement, see [`STACK.md`](STACK.md).
...
- [`STACK.md`](STACK.md) — layered architecture and OSS stack
- [`PRODUCT.md`](PRODUCT.md) — product pitch and DX
- [`ARCHITECTURE.md`](ARCHITECTURE.md) — extension points and adapter contracts
- [`OSS-AUDIT.md`](OSS-AUDIT.md) — OSS choices and remaining swaps
...
acceptance criteria) lives in [`POSITIONING.md`](POSITIONING.md) Part 4.
**That is the canonical execution doc.** This section gives the
phase-level summary; the executing agent works from POSITIONING.md.
...
redundant dashboard tabs; create `STACK.md` and `POSITIONING.md`.
...
positioning in POSITIONING.md Part 1 is aspirational.
...
See POSITIONING.md Part 4 §1a-1m for the 13 work-streams with
checkboxes.
...
See POSITIONING.md Part 4 §2 for the 11 items with checkboxes.
...
4. **AgentField Data in Dashboard** — ✅ shipped via link-out + inline
   summary. Per ARCHITECTURE.md's "don't rebuild what's already
...
See POSITIONING.md Part 4 §3 for the 5 items with checkboxes.
...
See POSITIONING.md Part 4 §4 for the 4 items with checkboxes.
...
2. **Within a phase, items are mostly independent** — multiple agents
   can parallelize.
3. **Tick boxes in POSITIONING.md Part 4** as items land. Don't track
   progress here in STRATEGY.md; this doc is for *intent*, POSITIONING
   is for *execution*.
